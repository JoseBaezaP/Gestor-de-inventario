<nav
  ...
  <a href="{{route('inventory')}}"
    class="{{request()->routeIs('inventory') ? 'active' : ''}} focus:outline-none hover:border-white hover:border-b-2">Mis productos</a>
  <a href="{{route('general')}}"
    class="{{request()->routeIs('general') ? 'active' : ''}} focus:outline-none hover:border-white hover:border-b-2">Inventario general</a>
  @if(auth()->user()->role)
